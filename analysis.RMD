---
title: "H.pylori Rhesus Microbiome Analysiss"
author: 
  - "Noah Siegel"
...
hpylori <- metadata %>%
  select(SampleID, Treatment, site, log10_cfu.gm)
p <- ggplot(hpylori, aes(x=Treatment, y=log10_cfu.gm, color=site))  + 
  geom_boxplot(alpha=0.8, size=1) +
  geom_point(alpha=0.8, size=4) +
  scale_color_manual(name="Site", labels = c("Lavage", "Buccal"), values=c("dark red", "dark blue")) +
  theme_classic(base_size = 28) +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  ggtitle("H.pylori Log10 CFU") +
  facet_wrap(~site + Treatment, scales= "free_x", nrow=1)
ggsave(plot=p,
       filename="results/hpylori-log10cfu.png",
       width=16,
       height=8,
       dpi=300)
p
...
#### Microbiota Composition at Class level
...
#### Microbiota Composition at Order level
...
#### Microbiota Composition at Family level



```{r echo=F}
t1 <- trans_abund$new(dataset = dataset, taxrank = "Family", ntaxa = 20)
p <- t1$plot_bar(use_colors=hitesh, others_color = "grey70", facet = c("Treatment","site"), xtext_keep = FALSE, legend_text_italic = FALSE) +
  theme_classic(base_size = 22) +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  ggtitle("Family")
...
#### Microbiota Composition at Genus level
...
#### Microbiota Composition at Species level
```{r echo=F}
...
p
```
The most abundance sequences at the species level were unidentified taxa. This not not mean there was no species level detection.
...
### IL8 and lavage genus abundance Spearman correlations separated by H. pylori status
```{r echo=F, cache=F}
#BAL
bp <- subset_samples(physeq, site=="BAL")
table <- as.data.frame(otu_table(bp))
...
### IL8 and lavage genus abundance Spearman correlations independent of H. pylori status
```{r echo=F}
...
### IL8 and lavage alpha diversity Spearman correlations inpendent of H. pylori status
```{r echo=F}
...
### IL8 and Buccal genus abundance Spearman correlations separated by H. pylori status
```{r echo=F, cache=F}
#Swab
bp <- subset_samples(physeq, site=="Swab")
table <- as.data.frame(otu_table(bp))
...
### IL8 and  Buccal genus abundance Spearman correlations independent of H. pylori status
```{r echo=F}
# calculate correlations for different groups using parameter by_group
t1$cal_cor(by_group = "site", use_data = "other", p_adjust_method = "fdr", other_taxa = t2$res_rf$Taxa[1:20])
# return t1$res_cor
...
### IL8 and Buccal alpha diversity Spearman correlations independent of H. pylori status
```{r echo=F}
dataset$cal_alphadiv(PD = TRUE)
t1 <- trans_alpha$new(dataset = dataset, group = "Site_status")
t1 <- trans_env$new(dataset = dataset, add_data = subsetted[, 7:8])
