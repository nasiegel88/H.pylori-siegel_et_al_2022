...
merge_meta <- metadata %>% 
  mutate(
    site = str_replace(site, "BAL", "Lung"),
    site = str_replace(site, "Swab", "Oral")
  ) %>%
  unite(Site_status, Treatment, site, remove=F)
...
model <- age_data %>%
  rename(
    ...
    `IL-8 Lung (pg)` = il_bal_pg.ml,
    `IL-8 Plasma (pg)` = il_plasma_pg.ml
  ) %>%
  select(HP_status, `Age (days)`, `Weight (kg)`, `IL-8 Lung (pg)`, `IL-8 Plasma (pg)`)

reg <- glm(as.factor(HP_status) ~ `Weight (kg)`  + `Age (days)` + `IL-8 Lung (pg)` + `IL-8 Plasma (pg)`, data=model, family=binomial)
p <- plot_model(reg,
             type="std2",
             group.terms = c(1, 2, 3, 4),
             vline.color="black", color=c("black"),) + 
labs(y = "") +
ggtitle("") +
  theme_classic() +
  theme(
...
no_duplicates <- metadata %>% filter(site=="Lung") # will remove duplicates
...
```{r cache=F, echo=F}
# lavage
bp <- subset_samples(physeq, site=="Lung")
...
```{r echo=F}
# buccal
bp <- subset_samples(physeq, site=="Oral")
...
p <- t1$plot_lefse_bar(LDA_score = 2) +
  ...
  scale_fill_manual(name="Status",
                    labels = c(expression(paste(italic("H. pylori"), "(-) Oral")),
                               expression(paste(italic("H. pylori"), "(+) Oral")),
                               expression(paste(italic("H. pylori"), "(-) Lung")),
                               expression(paste(italic("H. pylori"), "(+) Lung"))),
...
# lavage lefse
ps <- subset_taxa(physeq, !is.na(Species) & !Phylum %in% c("", "uncharacterized"))
bp <- subset_samples(ps, site=="Lung")
table <- as.data.frame(otu_table(bp))
taxa <- as.data.frame(tax_table(bp))
taxa$Kingdom  <- str_replace(taxa$Kingdom, "d__" , "k__")
taxa$Phylum   <- str_c("p_", taxa$Phylum  , sep="_")
...
data$tidy_dataset()
data$cal_abund()
data$sample_table <- subset(data$sample_table, site == "Lung")
...
p <- t1$plot_lefse_bar(LDA_score = 2) +
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5),
    axis.text.x=element_text(size=25),
    axis.text.y=element_text(size=19, angle = 0, hjust = 1),
    legend.text.align = 0,
    legend.text = element_text(hjust = 0)
  )
...
# buccal lefse
ps <- subset_taxa(physeq, !is.na(Species) & !Phylum %in% c("", "uncharacterized"))
bp <- subset_samples(ps, site=="Oral")
table <- as.data.frame(otu_table(bp))
taxa <- as.data.frame(tax_table(bp))
taxa$Kingdom  <- str_replace(taxa$Kingdom, "d__" , "k__")
taxa$Phylum   <- str_c("p_", taxa$Phylum  , sep="_")
...
data$tidy_dataset()
data$cal_abund()
data$sample_table <- subset(data$sample_table, site == "Oral")
...
scale_color_manual(name="Site", labels = c("Lung", "Oral"),
...
p <- t1$plot_group_distance(distance_pair_stat = T) +
            scale_color_manual(name="Site", values=c("dark red", "dark blue"),
                               labels=c('Lung', 'Oral')) +
  scale_x_discrete(labels=c('Lung','Oral'))  +
  theme_classic() +
  theme(
    text = element_text(size=30),
    plot.title = element_text(hjust = 0.5),
...
t1$res_cor$Type <- str_replace(t1$res_cor$Type, '_', ' ') 
...
p <- t1$plot_corr()  + 
  ...
  theme(
   ...
   geom_text(aes(label = Significance), color="white", size=10, position = position_nudge(y = -0.3))+
  labs(fill = "Spearman") 
...
p <- t1$plot_corr() + 
  ...
  theme(
   ...
   geom_text(aes(label = Significance), color="white", size=20, position = position_nudge(y = -0.3)) +
  labs(fill = "Pearson") 
